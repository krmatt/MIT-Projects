...
def compute_probabilities(X, theta, temp_parameter):
    ...
    c_arr = np.dot(theta,np.transpose(X)) / temp_parameter
    c = c_arr.max(axis=0)

    term1 = 1 / np.sum(np.exp(c_arr - c), axis=0)

    term2 = np.exp(c_arr - c)

    H = np.multiply(term1,term2)
    return H
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    num = np.exp(np.dot(theta,np.transpose(X)) / temp_parameter)
    den = np.sum(num, axis=0)
    log = np.nan_to_num(np.log(num/den))
    ...
    theta2 = (lambda_factor/2) * np.sum(np.square(theta))
...
#
#    gradj = (-1/(temp_parameter*n)) * np.matmul((M-p),X) + (lambda_factor*theta)
#    
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    n = X.shape[0]
    errors = 0
    y_mod3 = np.mod(get_classification(X, theta, temp_parameter),3)
    for i in range(n):
        if not Y[i] == y_mod3[i]:
            errors += 1
    test_error = errors/n
