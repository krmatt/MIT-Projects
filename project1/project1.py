...
def perceptron(feature_matrix, labels, T):
    ...
    #print('matrix shape:', feature_matrix.shape[1])
    theta_0 = 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            #print('feature_vector:', feature_matrix[i])
            #print('theta:', theta)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            n += 1
    avg_theta = np.divide(theta_sum,n)
    avg_theta_0 = np.divide(theta_0_sum,n)
...
def pegasos_single_step_update(
    ...
    if label * (np.dot(current_theta, feature_vector) + current_theta_0) <= 1:
        ...
        updated_theta_0 = current_theta_0 + label * eta
        return (updated_theta, updated_theta_0)
    else:
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    result = np.sign(np.dot(feature_matrix, theta) + theta_0)
...
def classifier_accuracy(
    ...
    """
    # Your code here
    model = classifier(train_feature_matrix, train_labels, **kwargs)
    train_preds = classify(train_feature_matrix, model[0], model[1])
    val_preds = classify(val_feature_matrix, model[0], model[1])
